(function() {
	...
	function localDataService($http, $q, _, ENV) {
		...
		var categoriesUrl = ENV.apiUrl + 'categories.json';
		...
		function getBusinessesByCategory(category, featured) {
			...
			return promise.then(function(businesses) {
				return _.filter(businesses, function(business) {
					var flag = category === 'All' || business.category === category;

					var isBusinessFeatured = business.featured == 'true' || business.featured == true;
					if (flag && featured && !isBusinessFeatured) {
						flag = false;
					}
