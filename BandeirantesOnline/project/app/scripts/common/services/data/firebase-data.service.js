	...
	function firebaseDataService(_, db, $firebaseArray, $firebaseObject) {
		...
		function getBusinesses() {
			...
			return $firebaseArray(query).$loaded()
				.then(initArray)
				.then(extendBusinessesWithCategory);
		}
		...
		function extendBusinessesWithCategory(businesses) {
			return getCategories().then(function(categories) {
				_.each(businesses, function(business) {
					var category = _.find(categories, { guid: business.category });
					business.categoryTitle = category.title;
				});
				return businesses;
			});
		}

		function extendBusinessWithCategory(business) {
			return getCategories().then(function(categories) {
				var category = _.find(categories, { guid: business.category });
				business.categoryTitle = category.title;
				return business;
		...
		function getBusiness(businessId) {
			...
			return $firebaseObject(query).$loaded()
				.then(initItem)
				.then(extendBusinessWithCategory)
		...
		function getCategories() {
			...
			return $firebaseArray(query).$loaded().then(initArray).then(function(categories) {
				return [{ title: 'All', guid: 'All' }].concat(_.sortBy(_.unique(categories)));
			});
		}
